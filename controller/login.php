...
if (isset($_POST['clientlogin'])) {
	...
	if (empty($Username) || empty($Password)) {
		header("location: ../user/index.php?msg=Username and Password is required");
		exit;
	} else {
		client();
	}
		...
		if (mysqli_num_rows($sql) > 0) {
			if (!empty($npass) && !empty($cpass)) {
				if ($npass == $cpass) {
					...
					header("location:../user/changepassword.php?id=Successfull...");
					exit;
				} else {
					header("location:../user/changepassword.php?msg=New password and Confirm pssword do not match?");
					exit;
				}
			} else {
	...
	if (empty($old)) {
		...
		header("location:../changepassword.php?msg=Old Password is required");
		exit;
	} else {
		...
		if (mysqli_num_rows($sql) > 0) {
			if (!empty($npass) && !empty($cpass)) {
				if ($npass == $cpass) {
					mysqli_query($db, "update employee set Password='$npass' where Password='$old' and Email='$email'");
					header("location:../changepassword.php?id=Successfull...");
					exit;
				} else {
					header("location:../changepassword.php?msg=New password and Confirm pssword do not match?");
					exit;
				}
			} else {
				header("location:../changepassword.php?msg=New Password And Confirm Password is required");
				exit;
			}
		} else {
			header("location:../changepassword.php?msg=Old Password is Wrong!");
			exit;
		}
	}
	...
	if (empty($Username) || empty($Password)) {
		...
		header("location:../index.php?msg=UserName and Password is required");
		exit;
	} else {
...
function client()
{
	...
	$sql = mysqli_query($db, "select * from owners where owner_email='$Username' AND password='$Password' ");
	...
	if (mysqli_num_rows($sql) > 0) {
		...
		session_start();
		$_SESSION['User'] = $row;
		$email = $_SESSION['User']['owner_email'];
		$roleid = $_SESSION['User']['RoleId'];
		header('location:../user/home.php');
		exit;
	} else {
		header("location:../user/index.php?msg=Username and Password is Wrong!");
		exit;
	}
}
function user()
